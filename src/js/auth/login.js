...
import { isSessionEnding } from "./session-manager.js";

const routed = await routeSignedInUser().catch(() => false);
if (!routed && !isSessionEnding()) {
  const form = document.querySelector("#login-form");
  const sessionNotice = {
    inactive: "You were signed out after 15 minutes of inactivity. Please sign in again.",
    maximum: "Your session reached its 8-hour limit. Please sign in again.",
    "verification-failed": "We couldn’t verify your session. Please sign in again when your connection is available.",
    "logout-incomplete": "You are signed out in this browser. We couldn’t confirm server sign-out; reconnect and sign in again to retry sign-out.",
  }[new URLSearchParams(location.search).get("session")];
  if (sessionNotice) showMessage(sessionNotice, "error");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try { await routeSignedInUser(); }
    catch { setBusy(submit, false); showMessage("We couldn’t verify your session. Please try again.", "error"); }
  });
}
